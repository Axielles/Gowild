var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Component, Input } from '@angular/core';
import { LeafletService } from '../../services/Leaflet.service';
import { Event } from '../../models/event';
var DetailsEventsComponent = /** @class */ (function () {
    function DetailsEventsComponent(leaflet) {
        this.leaflet = leaflet;
    }
    DetailsEventsComponent.prototype.ngOnInit = function () {
        this.leaflet.onCall(this.event.latlon);
    };
    DetailsEventsComponent.prototype.ngOnDestroy = function () {
        console.log('dans ngOnDestroy');
    };
    __decorate([
        Input(),
        __metadata("design:type", Event)
    ], DetailsEventsComponent.prototype, "event", void 0);
    __decorate([
        Input(),
        __metadata("design:type", Number)
    ], DetailsEventsComponent.prototype, "index", void 0);
    DetailsEventsComponent = __decorate([
        Component({
            selector: 'app-details-events',
            templateUrl: './details-events.component.html',
            styleUrls: ['./details-events.component.css']
        }),
        __metadata("design:paramtypes", [LeafletService])
    ], DetailsEventsComponent);
    return DetailsEventsComponent;
}());
export { DetailsEventsComponent };
//# sourceMappingURL=details-events.component.js.map